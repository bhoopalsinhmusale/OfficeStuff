...
//	boolean isBinary;
	...
	private void checkData2Xml(String[] d) throws FileNotFoundException,
		...
		for (int tf : TAG_FORMATS) {
			...
			byte[] doc;
			if ("1".equals(d[3])) {
				doc = data2xml1(dataName, copybookName, "Y".equalsIgnoreCase(d[3]), d[4], tf);
			} else if ("1a".equals(d[3])) {
				doc = data2xml1a(dataName, copybookName, "Y".equalsIgnoreCase(d[3]), d[4], tf);
			} else if ("1b".equals(d[3])) {
				doc = data2xml1b(dataName, copybookName, "Y".equalsIgnoreCase(d[3]), d[4], tf);
			} else if ("2".equals(d[3])) {
				doc = data2xml2(dataName, copybookName, "Y".equalsIgnoreCase(d[3]), d[4], tf);
			} else {
				doc = data2xml3(dataName, copybookName, "Y".equalsIgnoreCase(d[3]), d[4], tf);
			}
